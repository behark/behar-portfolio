...
const plans = [
    ...
    },
    {
        ...
        period: '/muaj',
        features: [
            'Monitorim të gjitha burimet',
            'Njoftime Telegram & WhatsApp',
            'Dashboard i plotë',
            'Zbulim çmimesh',
...
export function CarSniperPricing() {
    const handleGetStarted = (planName: string, price: string) => {
        const message = encodeURIComponent(
            `Përshëndetje! Jam i interesuar për planin ${planName} (${price}/muaj) të Car Sniper.`
        );
                        ...
                        <ScrollReveal key={index} direction="up" delay={index * 0.15}>
                            <div
                                className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 ${
                                    plan.popular
                                        ? 'border-2 border-blue-600 dark:border-blue-500 relative scale-105'
